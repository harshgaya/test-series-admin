...
import { MdUpload, MdClose, MdInsertDriveFile, MdImage, MdAudiotrack, MdVideoLibrary, MdCheckCircle } from 'react-icons/md'
...
export default function FileUpload({ type, label, value, onChange }) {
  ...
  const colors = COLOR_MAP[type]  || { bg: '#F9FAFB', icon: '#6B7280', border: '#E5E7EB' }
  ...
  async function handleFileChange(e) {
    ...
    try {
      ...
      // Step 2 — upload directly to R2 with XHR for progress
      ...
      // Step 3 — set public URL in parent form
      onChange(publicUrl)
  ...
  return (
    <div>
      ...
      {value && !uploading ? (
        ...
        >
          <MdCheckCircle style={{ color: colors.icon, fontSize: 20, flexShrink: 0 }} />
        ...
        >
          ...
          <div className="flex-1 text-left">
            <p className="text-sm font-medium text-gray-700">
              {uploading ? `Uploading... ${progress}%` : `Click to upload ${label.toLowerCase()}`}
            </p>
      ...
      {uploading && (
        <div className="mt-2">
          <div className="flex items-center justify-between mb-1">
            ...
            <span className="text-xs font-medium" style={{ color: colors.icon }}>{progress}%</span>
          </div>
