...
export default function CrashCoursesClient({ initialCourses, exams }) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      const res = await fetch(
        isEdit ? `/api/crash-courses/${selected.id}` : "/api/crash-courses",
        {
      ...
      const enriched = {
        ...data.data,
        exam: { name: exam?.name },
        _count: selected?._count || { courseTests: 0, enrollments: 0 },
      };
      if (isEdit) {
        setCourses(
          courses.map((c) =>
  ...
  async function handleDelete() {
    setLoading(true);
    try {
      const res = await fetch(`/api/crash-courses/${selected.id}`, {
        method: "DELETE",
      });
      const data = await res.json();
      if (!data.success) {
      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">
            30/60/90 day structured test series for exam preparation
          </p>
        </div>
        <button
          onClick={() => {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {courses.map((course) => (
            <div key={course.id} className="card p-5">
              ...
              <Link
                href={`/admin/crash-courses/${course.id}`}
                className="font-bold text-gray-900 hover:underline mb-1 block"
                style={{ color: "#0F172A" }}
              >
                {course.title}
              </Link>
              ...
              <div className="flex items-center gap-2 flex-wrap mb-3">
                ...
                <Badge
                  ...
                  color={Number(course.price) === 0 ? "green" : "purple"}
                />
                <Badge
      ...
      >
        <div className="space-y-4">
          ...
          <div>
            ...
            <input
              className="input-field"
              placeholder="e.g. NEET 2026 — 30 Day Crash Course"
              value={form.title}
