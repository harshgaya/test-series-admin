...
import { MdAttachMoney, MdTrendingUp, MdCalendarToday, MdDownload } from 'react-icons/md'
import Badge from '@/components/ui/Badge'
import Pagination from '@/components/ui/Pagination'
import EmptyState from '@/components/ui/EmptyState'
import { PAYMENT_STATUSES } from '@/lib/constants'

const STATUS_COLORS = { PENDING: 'yellow', SUCCESS: 'green', FAILED: 'red', REFUNDED: 'purple' }
...
export default function RevenueClient() {
  ...
  const [summary, setSummary]       = useState({ monthRevenue: 0, yearRevenue: 0, totalRevenue: 0 })
  ...
  function exportCSV() {
    ...
    const headers = ['Date', 'Student', 'Contact', 'Test', 'Amount', 'Status', 'Order ID']
    ...
    <>
        ...
        <div className="card p-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xl font-bold text-gray-900">{formatCurrency(summary.monthRevenue)}</p>
            </div>
          </div>
        </div>
        <div className="card p-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xl font-bold text-gray-900">{formatCurrency(summary.yearRevenue)}</p>
            </div>
          </div>
        </div>
        <div className="card p-5">
      ...
      <div className="page-header">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <input type="date" className="input-field w-36 py-2" value={filterFrom}
            onChange={e => { setFilterFrom(e.target.value); setPage(1) }} />
          <span className="text-gray-400 text-sm">to</span>
          <input type="date" className="input-field w-36 py-2" value={filterTo}
      ...
      <div className="card overflow-hidden">
        {loading ? (
          ...
          <EmptyState title="No payments found" message="Payments will appear here after students purchase tests" />
        ) : (
          <>
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  {['Date', 'Student', 'Test', 'Amount', 'Order ID', 'Status'].map(h => (
                ...
                {payments.map(p => (
                  <tr key={p.id} className="table-row-hover">
                    <td className="px-4 py-3 text-gray-500 text-xs">
                      {new Date(p.createdAt).toLocaleDateString('en-IN')}
                      <br />
                      <span className="text-gray-400">{new Date(p.createdAt).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' })}</span>
                    </td>
                    <td className="px-4 py-3">
                      <p className="font-medium text-gray-900">{p.student?.name}</p>
                      <p className="text-xs text-gray-400">{p.student?.email || p.student?.phone}</p>
                    </td>
                    <td className="px-4 py-3 text-gray-600 max-w-xs">
                      <p className="truncate">{p.test?.title || '—'}</p>
                    </td>
                    <td className="px-4 py-3">
                      <span className={`font-bold ${p.status === 'SUCCESS' ? 'text-green-600' : 'text-gray-700'}`}>
                        ₹{p.amount}
                      </span>
                    </td>
