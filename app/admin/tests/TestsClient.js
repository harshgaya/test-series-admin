...
import { TEST_TYPES, TEST_STATUSES, STUDENT_APP_URL } from "@/lib/constants";
...
export default function TestsClient({ exams }) {
  ...
  function shareLink(t) {
    // Uses the hashid (publicId) returned by the API. Falls back to null.
    if (!t.publicId) return null;
    return `${STUDENT_APP_URL}/test/${t.publicId}`;
  }

  function copyLink(t) {
    const link = shareLink(t);
    if (!link) {
      toast.error("Share link not available for this test");
      return;
    }
