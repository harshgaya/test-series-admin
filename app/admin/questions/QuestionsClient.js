...
export default function QuestionsClient({ filters }) {
  ...
  async function handleDelete() {
    setDelLoading(true)
    try {
      const res  = await fetch(`/api/questions/${selected.id}`, { method: 'DELETE' })
      const data = await res.json()
  ...
  function truncate(text, len = 80) {
    if (!text) return ''
    const plain = text.replace(/\$[^$]*\$/g, '[math]').replace(/\\\w+\{[^}]*\}/g, '[math]')
  ...
  return (
    <>
      ...
      <div className="page-header flex-wrap gap-3">
        <div>
          <p className="page-title">Questions ({total.toLocaleString('en-IN')})</p>
      ...
      <div className="card p-4 mb-4">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <select className="input-field w-32 py-1.5" value={filterExam}
            onChange={e => { setFilterExam(e.target.value); setFilterSub(''); setFilterCh(''); setPage(1) }}>
            <option value="">All Exams</option>
            {exams.map(e => <option key={e.id} value={e.id}>{e.name}</option>)}
          </select>

          <select className="input-field w-32 py-1.5" value={filterSub}
            onChange={e => { setFilterSub(e.target.value); setFilterCh(''); setPage(1) }}>
            <option value="">All Subjects</option>
            {filteredSubjects.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
          </select>

          <select className="input-field w-36 py-1.5" value={filterCh}
            onChange={e => { setFilterCh(e.target.value); setPage(1) }}>
            <option value="">All Chapters</option>
            {filteredChapters.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
          </select>

          <select className="input-field w-28 py-1.5" value={filterDiff}
            onChange={e => { setFilterDiff(e.target.value); setPage(1) }}>
            <option value="">Difficulty</option>
            {DIFFICULTIES.map(d => <option key={d.value} value={d.value}>{d.label}</option>)}
          </select>

          <select className="input-field w-36 py-1.5" value={filterType}
      ...
      <div className="card overflow-hidden">
        {loading ? (
          ...
          <EmptyState title="No questions found" message="Try adjusting filters or add new questions"
            action={<Link href="/admin/questions/new" className="btn-primary"><MdAdd />Add Question</Link>} />
        ) : (
          <>
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Chapter</th>
                ...
                {questions.map((q, i) => (
                  <tr key={q.id} className="table-row-hover">
                    ...
                    <td className="px-4 py-3 max-w-xs">
                      <p className="text-gray-900 text-sm leading-snug">{truncate(q.questionText)}</p>
                      {q.questionImageUrl && (
                        <span className="text-xs text-blue-500 mt-0.5">📷 Has image</span>
                      )}
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <Badge
                        label={QUESTION_TYPES.find(t => t.value === q.questionType)?.label || q.questionType}
                        color="blue"
                      />
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3 text-gray-500 text-xs">{q.usageCount} tests</td>
                    <td className="px-4 py-3">
                      <Badge label={q.isActive ? 'Active' : 'Inactive'} color={q.isActive ? 'green' : 'gray'} />
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-1">
                        <Link href={`/admin/questions/${q.id}`}
                          className="p-1.5 text-gray-400 hover:text-blue-600 transition-colors" title="View/Edit">
