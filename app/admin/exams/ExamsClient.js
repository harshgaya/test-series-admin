...
export default function ExamsClient({ initialExams, tracks }) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (isEdit) {
        setExams(
          exams.map((e) => (e.id === selected.id ? { ...e, ...data.data } : e)),
        );
        toast.success("Exam updated!");
      } else {
        setExams([
          ...exams,
          { ...data.data, _count: { subjects: 0, questions: 0, tests: 0 } },
        ]);
  ...
  async function handleDelete() {
    setLoading(true);
    try {
      const res = await fetch(`/api/exams/${selected.id}`, {
        method: "DELETE",
      });
      const data = await res.json();
      if (!data.success) {
  ...
  async function toggleActive(exam) {
    try {
      ...
      setExams(
        exams.map((e) =>
          e.id === exam.id ? { ...e, isActive: !e.isActive } : e,
        ),
      );
  ...
  return (
    <>
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">
            Add and manage exams like NEET, JEE, EAMCET
          </p>
        </div>
      ...
      <Modal
        ...
        subtitle={
          selected ? `Editing ${selected.name}` : "Fill in the details below"
        }
      >
